...
class HomepageController < ApplicationController
  ...
  def find_listings(params, listings_per_page, filter_params, includes, location_search_in_use, keyword_search_in_use)
    ...
    location_search_hash = location_search_params(
      ...
      location_search_in_use && keyword_search_in_use,
  ...
  def location_search_params(latlng, distance_unit, distance_max, minimum_distance_max, use_scale, limit_by_distance)
    ...
    if @view_type != 'map'
      Maybe(latlng)
        .map {
          ...
          scale_or_distance = use_scale ? { scale: distance * 0.75 } : { sort: :distance }
